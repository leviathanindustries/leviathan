...
stalker_company = new API.collection index: stalker, type: company
stalker_source = new API.collection index: stalker, type: source


API.add 'service/stalker/sources',
  get:
    return stalker_source.search this.queryParams

API.add 'service/stalker/companies',
  get:
    return stalker_company.search this.queryParams
...
API.service.stalker.company = (company) ->
  # get details about the company

API.service.stalker.scrape = (url) ->
  s = stalker_source.find url:url
  stalker_source.insert({url:url}) if not s
  ...
  for entity in ggl.entities
    if entity.type is 'ORGANIZATION'
      # is the company already in stalker_company? If not:
...
API.service.stalker.read = (urls) ->
  if not urls?
    urls = []
    stalker_source.each (src) ->
      urls.push src.url
...
API.service.stalker.eod = (types=['LSE','NASDAQ','AMEX','NYSE','ASX'], load=false, update=false, delay=1) ->
  results = []
      ...
      for r in res
        if r.Code and r.Code.length and r.Name and r.Name.length
          ...
          if exists and load and update
            ...
            company._id = stalker_company.insert company
          results.push company
  return {count:results.length, data:results}



API.service.stalker.loadcompanyurls = (update) ->
  ...
  companies = stalker_company.find matcher
  found = 0
  companies.forEach (company) ->
    ...
    try
      ...
      if url
        stalker_company.update company._id, {url:url}
        found += 1
  return found

API.service.stalker.loadcompanyabstract = (update) ->
