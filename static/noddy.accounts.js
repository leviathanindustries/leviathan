...
noddy.dev = window.location.host.indexOf('dev.') === -1 ? false : true;
noddy.api = noddy.dev ? 'https://dev.lvatn.com' : 'https://api.lvatn.com';
...
noddy.failureCallback = function(data, action='after') {
  $('.noddyLoading').hide();
  ...
  if (typeof noddy[action+'Failure'] === 'function') noddy[action+'Failure']();
}
...
noddy.token = function(e) {
  ...
  $('#noddyToken').attr('placeholder','Enter your token (sent to ' + noddy.user.email + ')');
  var opts = {
    type:'POST',
    cache: false,
...
noddy.login = function(e) {
  ...
  $('#noddyEmail').unbind('keyup').bind('keyup',function(e) { if (e.keyCode === 13) { noddy.token(); } });
  ...
  if ( window.location.hash.indexOf('access_token=') !== -1 ) {
    ...
    if (window.location.search.indexOf('apikey=') !== -1) data.apikey = window.location.search.split('apikey=')[1].split('&')[0]
    ...
    opts.data = JSON.stringify(data);
    if ( data.email || data.hash || data.apikey ) {
      $('.noddyLoading').show();
      $.ajax(opts);
    } else {
      $('.noddin').hide();
